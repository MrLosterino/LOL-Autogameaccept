...
def find_lockfile():
    ...
    standard_paths = [
        ...
        "I:/Riot Games/League of Legends/Lockfile",
        "J:/Riot Games/League of Legends/Lockfile",
        "K:/Riot Games/League of Legends/Lockfile",
        "L:/Riot Games/League of Legends/Lockfile",
        "M:/Riot Games/League of Legends/Lockfile",
        "N:/Riot Games/League of Legends/Lockfile",
        "O:/Riot Games/League of Legends/Lockfile",
        "P:/Riot Games/League of Legends/Lockfile",
        
        "X:/Riot Games/League of Legends/Lockfile",
        "Z:/Riot Games/League of Legends/Lockfile",
...
def get_lcu_credentials():
    ...
    # Debugging
    print(f"[DEBUG] Lockfile ausgelesen: Port={port}, Passwort={password}")
...
def start_ingame_timer():
    start_time = time.time()
    while script_running:
        elapsed_time = int(time.time() - start_time)
        minutes = elapsed_time // 60
        seconds = elapsed_time % 60
        print(f"Ingame-Zeit: {minutes:02}:{seconds:02}")
        time.sleep(1)
        
def get_ingame_time(base_url, headers):
    """Ruft die aktuelle Spielzeit in Sekunden ab."""
    url = f"{base_url}/lol-gameflow/v1/session"
    try:
        response = requests.get(url, headers=headers, verify=False)
        if response.status_code == 200:
            data = response.json()
            if "gameData" in data and "gameTime" in data["gameData"]:
                game_time_seconds = int(data["gameData"]["gameTime"])
                minutes = game_time_seconds // 60
                seconds = game_time_seconds % 60
                return f"{minutes:02}:{seconds:02}"  # Format MM:SS
        return None
    except requests.exceptions.RequestException as e:
        print(f"[ERROR] Ausnahme beim Abrufen der Spielzeit: {e}")
        return None
...
def monitor_matchmaking(status_label):
    ...
    while script_running:
        ...
        if gameflow_status == "ReadyCheck":
            ...
            print("[INFO] Spieler ist im Spiel. Zeige Ingame-Zeit...")
            while gameflow_status == "InProgress" and script_running:
                game_time = get_ingame_time(base_url, headers)
                if game_time:
                    status_label.config(text=f"Status: Im Spiel\nZeit: {game_time}", bg="green")
                else:
                    status_label.config(text="Status: Im Spiel\nZeit: Unbekannt", bg="green")
                time.sleep(2)
                gameflow_status = get_gameflow_status(base_url, headers)
